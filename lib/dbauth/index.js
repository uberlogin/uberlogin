...
module.exports = function (config, userDB, couchAuthDB) {
  ...
  this.createDB = function (dbName) {
    var serverNameWithAuth = util.getDBURL(config.getItem('dbServer'));
    return pouchdbUtil.createDB(serverNameWithAuth, dbName);
  };

  this.removeDB = function(dbName) {
    var serverNameWithAuth = util.getDBURL(config.getItem('dbServer'));
    return pouchdbUtil.removeDB(serverNameWithAuth, dbName);
  };
